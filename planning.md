...
You can do this with regex or by asking the LLM to return a JSON object with those three keys.

**Chosen approach: LLM with regex fallback** (implemented as `_parse_query()` in [agent.py](agent.py)). An LLM call (Groq, `llama-3.3-70b-versatile`) was chosen as the primary parser because it handles the varied ways users phrase queries more robustly than regex — it correctly canonicalizes word-based sizes after the word "size" (e.g., `"size medium"` → `"M"` instead of the raw string `"medium"`), extracts multi-word size phrases (e.g., `"size extra small"` → `"XS"` instead of capturing only `"extra"`), and understands price mentions without a `$` sign (e.g., `"30 dollars"` → `30`, which the bare-regex fallback would miss). The LLM is called with `temperature=0.0` for deterministic output.

A deterministic regex parser (`_parse_query_regex()` in [agent.py](agent.py)) is preserved as a fallback that runs automatically if the LLM call fails for any reason — missing API key, network error, malformed JSON response, etc. This means the agent still works offline or when the Groq API is unavailable.

The LLM is given a structured prompt with few-shot examples and asked to return a JSON object with three keys:

- **`description`** — meaningful item keywords with filler words stripped. The LLM identifies the core item the user wants (colors, materials, styles, item type) and discards phrases like "looking for", "find me", "I want", etc. For `"looking for a vintage graphic tee under $30"` this yields `"vintage graphic tee"`.
- **`size`** — canonicalized to standard codes when possible (`"small"`→`"S"`, `"medium"`→`"M"`, `"extra large"`→`"XL"`), kept as-is for raw/numeric sizes (`"W30"`, `"S/M"`, `"XXS"`, `"US 7"`), or `null` if no size is mentioned.
- **`max_price`** — the budget ceiling as a float, extracted from cues like `"under $30"`, `"max 50"`, `"cheaper than 25"`, `"less than 100"`, `"below 40"`, `"< 20"`, `"around 30"`, or bare `"$30"`. Set to `null` if no price is mentioned.

The regex fallback uses the same strip-as-you-go strategy: first look for a price cue or bare `$NN` amount, then `size <token>` or a word-based size, then use whatever text remains (minus filler words) as the description.
...
2. Parses the query (LLM with regex fallback, via `_parse_query()`) and stores `session["parsed"]`
